...
func CreateFirstSegmentPipeOnAllHosts(oid string, c *cluster.Cluster, fpInfo filepath.FilePathInfo) {
	...
	c.CheckClusterError(remoteOutput,
		"Unable to create segment data pipes",
		func(contentID int) string {
...
func VerifyHelperVersionOnSegments(version string, c *cluster.Cluster) {
	remoteOutput := c.GenerateAndExecuteCommand(
		"Verifying gpbackup_helper version",
		cluster.ON_HOSTS,
		func(contentID int) string {
	...
	c.CheckClusterError(remoteOutput,
		"Could not verify gpbackup_helper version",
		func(contentID int) string {
	...
	for contentID := range remoteOutput.Commands {
		segVersion := strings.TrimSpace(remoteOutput.Commands[contentID].Stdout)
		segVersion = strings.Split(segVersion, " ")[2] // Format is "gpbackup_helper version [version string]"
		if segVersion != version {
			gplog.Verbose("Version mismatch for gpbackup_helper" +
				" on segment %d on host %s: Expected version %s, found version %s.",
				contentID, c.GetHostForContent(contentID), version, segVersion)
			numIncorrect++
		}
	}
	if numIncorrect > 0 {
		cluster.LogFatalClusterError(
			"The version of gpbackup_helper must match " +
				"the version of gpbackup/gprestore, but found gpbackup_helper" +
				" binaries with invalid version",
				cluster.ON_HOSTS, numIncorrect)
	}
}

func StartGpbackupHelpers(c *cluster.Cluster, fpInfo filepath.FilePathInfo,
	operation string, pluginConfigFile string, compressStr string, onErrorContinue bool) {
	...
	remoteOutput := c.GenerateAndExecuteCommand(
		"Starting gpbackup_helper agent",
		cluster.ON_SEGMENTS,
		func(contentID int) string {
			...
			backupFile := fpInfo.GetTableBackupFilePath(contentID,0,
				GetPipeThroughProgram().Extension,true)
			helperCmdStr := fmt.Sprintf(
				"gpbackup_helper %s --toc-file %s --oid-file %s " +
					"--pipe-file %s --data-file %s --content %d%s%s%s",
					operation, tocFile, oidFile, pipeFile, backupFile, contentID,
					pluginStr, compressStr, onErrorContinueStr)
			// we run these commands in sequence to ensure that any failure is critical;
			// the last command ensures the agent process was successfully started
			return fmt.Sprintf(
				`cat << HEREDOC > %[1]s && chmod +x %[1]s && ( nohup %[1]s &> /dev/null &)
#!/bin/bash
...
`, scriptFile, gphomePath, helperCmdStr)
	...
	c.CheckClusterError(remoteOutput,
		"Error starting gpbackup_helper agent",
		func(contentID int) string {
...
func CleanUpHelperFilesOnAllHosts(c *cluster.Cluster, fpInfo filepath.FilePathInfo) {
	remoteOutput := c.GenerateAndExecuteCommand(
		"Removing oid list and helper script files from segment data directories",
		cluster.ON_SEGMENTS,
		func(contentID int) string {
	...
	errMsg := fmt.Sprintf("Unable to remove segment helper file(s)." +
		" See %s for a complete list of segments with errors and remove manually.",
		gplog.GetLogFilePath())
	c.CheckClusterError(remoteOutput, errMsg,
		func(contentID int) string {
			errorFile := fmt.Sprintf("%s_error", fpInfo.GetSegmentPipeFilePath(contentID))
			return fmt.Sprintf("Unable to remove helper file %s " +
				"on segment %d on host %s", errorFile, contentID, c.GetHostForContent(contentID))
	}, true)
}

func CleanUpSegmentHelperProcesses(c *cluster.Cluster, fpInfo filepath.FilePathInfo, operation string) {
	remoteOutput := c.GenerateAndExecuteCommand(
		"Cleaning up segment agent processes",
		cluster.ON_SEGMENTS,
		func(contentID int) string {
			tocFile := fpInfo.GetSegmentTOCFilePath(contentID)
			 ...
			 */
			return fmt.Sprintf("PIDS=`ps ux | grep \"%s\" | grep -v grep |" +
				" awk '{print $2}'`; if [[ ! -z \"$PIDS\" ]]; then kill $PIDS; fi", procPattern)
	})
	c.CheckClusterError(remoteOutput,
		"Unable to clean up agent processes",
		func(contentID int) string {
...
func CheckAgentErrorsOnSegments(c *cluster.Cluster, fpInfo filepath.FilePathInfo) error {
	remoteOutput := c.GenerateAndExecuteCommand(
		"Checking whether segment agents had errors",
		cluster.ON_SEGMENTS,
		func(contentID int) string {
			errorFile := fmt.Sprintf("%s_error", fpInfo.GetSegmentPipeFilePath(contentID))
			/*
	...
	numErrors := 0
	for contentID := range remoteOutput.Commands {
		if strings.TrimSpace(remoteOutput.Commands[contentID].Stdout) == "error" {
			gplog.Verbose("Error occurred with helper agent on" +
				" segment %d on host %s.", contentID, c.GetHostForContent(contentID))
			numErrors++
		}
	}
	if numErrors > 0 {
		helperLogName := fpInfo.GetHelperLogPath()
		return errors.Errorf("Encountered errors with %d helper agent(s)." +
			" See %s for a complete list of segments with errors, and see %s" +
			" on the corresponding hosts for detailed error messages.",
